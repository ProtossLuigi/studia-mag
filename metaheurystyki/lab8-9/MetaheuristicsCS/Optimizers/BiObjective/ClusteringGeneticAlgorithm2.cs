...
{
    class ClusteringGeneticAlgorithm2<Element> : GeneticAlgorithm<Element>
    {
        private readonly int no_clusters;
        private readonly Random rng;

        public ClusteringGeneticAlgorithm2(IEvaluation<Element, Tuple<double, double>> evaluation, IStopCondition stopCondition,
        ...
        protected override void Crossover()
        {
            ...
            for (int i = 0; i < population.Count - 1; i += 2)
            {
                ...
                int index2 = GetClosest(index1, clusters[cluster]);
        ...
        private int GetClosest(int index1, List<int> cluster)
        {
            double angle0 = Math.Atan(population[index1].Fitness.Item2 / population[index1].Fitness.Item1);
            int closest = index1;
            ...
            foreach(int i in cluster)
            {
                if(i != index1)
                {
                    double angle = Math.Atan(population[i].Fitness.Item2 / population[i].Fitness.Item1);
                    if(closest == index1 || Math.Abs(angle0 - angle) < min_delta)
                    {
                        closest = i;
                        min_delta = angle;
                    }
